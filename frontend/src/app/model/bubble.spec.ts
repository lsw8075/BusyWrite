...
describe('LeafBubble', () => {
  describe('when create leaf bubble', () => {
    ...
    beforeEach(() => {
      ...
      leafBubble = new LeafBubble(id, parentBubble, content, ownerId);
    });
  ...
  describe('when create leaf bubble without owner and content', () => {
    ...
    beforeEach(() => {
      id = 1;
      parentBubble = null;
      leafBubble = new LeafBubble(id, parentBubble);
    });
...
describe('InternalBubble', () => {
  ...
  beforeEach(() => {
    ...
    location = 0;
    parentBubble = null;
    internalBubble = new InternalBubble(id, parentBubble);
  ...
  it('getHeight should return correct height', () => {
    expect(internalBubble.getHeight()).toBe(1);
  });

  describe('when add wrong children to bubble', () => {
    beforeEach(() => {
      internalBubble.addChildren(leafBubble1, leafBubble2, internalBubble3, leafBubble4, leafBubble5, leafBubble6);
    });

    it('should not add bubble to child if its parent is differnt', () => {
      const children = internalBubble.childBubbles;
      expect(children).toContain(leafBubble1);
      expect(children).toContain(leafBubble2);
      expect(children).toContain(internalBubble3);
      expect(children).not.toContain(leafBubble4);
      expect(children).not.toContain(leafBubble5);
      expect(children).toContain(leafBubble6);
    });
  });

  describe('when adding correct children to bubble', () => {
    beforeEach(() => {
      internalBubble.addChildren(leafBubble1, leafBubble2, internalBubble3, leafBubble6);
      internalBubble3.addChildren(leafBubble4, leafBubble5);
    });

    it('internal bubbles should have correct childs', () => {
      const children1 = internalBubble.childBubbles;
      expect(children1).toContain(leafBubble1);
      expect(children1).toContain(leafBubble2);
      expect(children1).toContain(internalBubble3);
      expect(children1).toContain(leafBubble6);

      const children2 = internalBubble3.childBubbles;
      expect(children2).toContain(leafBubble4);
      expect(children2).toContain(leafBubble5);
    });
